    ...
    for (int i = 0; i < pixels; i++) {
        for (int j = 0; j < patch_size; j++) {
            for (int k = 0; k < patch_size; k++) {
                if (k != patch_size - 1)
                    printf("%0.5f ", patches[i * total_patch_size + j * patch_size + k]);
                else
                    printf("%0.5f", patches[i * total_patch_size + j * patch_size + k]);
            }
            printf("\n");
...
void print_patch_file(FILE* f, float* patches, int patch_size, int pixels)
{
    ...
    for (int i = 0; i < pixels; i++) {
        for (int j = 0; j < patch_size; j++) {
            for (int k = 0; k < patch_size; k++) {
                if (k != patch_size - 1)
                    fprintf(f, "%0.5f ", patches[i * total_patch_size + j * patch_size + k]);
                else
                    fprintf(f, "%0.5f", patches[i * total_patch_size + j * patch_size + k]);
            }
            fprintf(f, "\n");
...
float* create_patches(float* image, int patch_size, int m, int n)
{
    ...
    for (int i = 0; i < m * n * total_patch_size; i++)
        patches[i] = OUT_OF_BOUNDS;
...
float* euclidean_distance_matrix(float* patches, int patch_size, int rows, int cols)
{
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            distance[i * cols + j] =
                euclidean_distance_patch(patches + i * total_patch_size, patches + j * total_patch_size, patch_size);
        }
    }
...
void euclidean_distance_matrix_per_pixel(float* weights, float* patches, int patch_size, int pixel, int cols)
{
    ...
    for (int j = 0; j < cols; j++) {
        weights[j] =
            euclidean_distance_patch(patches + pixel * total_patch_size, patches + j * total_patch_size, patch_size);
    }
}
