...
__global__ void filtering(float* patches,
                          int patch_size,
                          float filt_sigma,
                          float* noise_image,
...
__device__ void
euclidean_distance_matrix_per_pixel(float* weights, float* patches, int patch_size, int pixel, int cols);
...
__host__ void
copy_row2padded(float* destination, float* source, int row_dest_idx, int row_source_idx, int size, int col_pad);

__host__ void
copy_col2padded(float* destination, float* source, int col_dest_idx, int col_source_idx, int size, int offset);
